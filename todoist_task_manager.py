...
def process_single_file(task_file: str, todoist_client: TodoistClient) -> int:
    ...
    try:
        ...
        total_operations = (
            len(updates) + len(completions) + len(deletions) + len(new_tasks)
        )
        ...
        print(
            f"📋 Found {len(updates)} updates, {len(completions)} completions, {len(deletions)} deletions, {len(new_tasks)} new tasks"
        )
        ...
        confirm = (
            input(f"Apply these changes from {task_file}? (y/n): ").lower().strip()
        )
...
def process_task_operations():
    ...
    print(
        "Features: Modular architecture, enhanced error handling, smart file management"
    )
    print()
    ...
    print(
        f"🎉 COMPLETE: Successfully processed {total_success} operations across {len(task_files)} {files_text}!"
    )
