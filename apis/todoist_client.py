...
class TodoistClient(BaseAPIClient):
    ...
    def find_task_by_content(self, tasks: List[Dict[str, Any]], content: str,
        ...
        if project_id:
            project_matches = [task for task in matches if task.get('project_id') == project_id]
            if len(project_matches) == 1:
                print(f"✅ Disambiguated by project ID")
        ...
        if section_id:
            section_matches = [task for task in matches if task.get('section_id') == section_id]
            if len(section_matches) == 1:
                print(f"✅ Disambiguated by section ID")
        ...
        print(f"⚠️  Multiple tasks still match after filtering:")
        for i, task in enumerate(matches[:3], 1):  # Show first 3
            ...
            print(f"   {i}. ID: {task_id_display}, Project: {project}, Section: {section}")

        if len(matches) > 3:
            print(f"   ... and {len(matches) - 3} more")

        print(f"⚠️  Using first match. For better accuracy:")
        print(f"   • Ensure task names are unique, OR")
        print(f"   • Specify 'project_name' and 'section_name' in your task JSON")
    ...
    def process_task_operation(self, task_info: Dict[str, Any], operation_type: str, 
        ...
        if section_name and project_name and lookup_project_id:
            if lookup_project_id in section_map and section_name in section_map[lookup_project_id]:
