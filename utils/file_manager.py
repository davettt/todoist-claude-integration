    ...
    for i, filename in enumerate(files):
        ...
        if "error" in preview:
            ...
            if preview["updates"] > 0:
                operations.append(
                    f"{preview['updates']} update{'s' if preview['updates'] != 1 else ''}"
                )
            if preview["deletions"] > 0:
                operations.append(
                    f"{preview['deletions']} deletion{'s' if preview['deletions'] != 1 else ''}"
                )
            if preview["new_tasks"] > 0:
                operations.append(
                    f"{preview['new_tasks']} new task{'s' if preview['new_tasks'] != 1 else ''}"
                )
