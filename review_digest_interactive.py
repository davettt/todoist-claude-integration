        ...
        if success:
            ...
            if action != 'skip':
                if gmail_id:
                    ...
                    print("⚠️  Cannot perform action - Gmail ID not available in this digest")
...
def show_email_with_analysis(email: dict, current: int, total: int):
    ...
    confidence = email.get('confidence', 'unknown').strip('* ')  # Remove stray asterisks
    ...
    try:
        ...
        if action == 'a':
            ...
            if success:
                ...
                print("   (Will auto-delete in 30 days, or restore from trash if needed)")
            else:
    ...
    try:
        ...
        for line in lines:
            ...
            # Gmail ID comment
                ...
                gmail_id = line_stripped.replace("<!-- gmail_id:", "").replace("-->", "").strip()
            ...
            elif (line_stripped.startswith("**AI Analysis:**") or
                  line_stripped.startswith("**Why low priority:**")) and current_email:
...
def show_summary(tracker: EmailFeedbackTracker, rated_count: int):
    ...
    print(f"  • Total feedback collected: {stats.get('total_feedback_count', 0)} emails")
