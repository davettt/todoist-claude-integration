...
def process_calendar_operations_file(calendar_file: str, calendar_client: GoogleCalendarClient) -> int:
    ...
    try:
        ...
        total_operations = len(create_events) + len(update_events) + len(delete_events) + len(time_blocks)
        
        if total_operations == 0:
            print("⚠️ No calendar operations found in this file!")
            return 0
        
        print(f"📋 Found {len(create_events)} creates, {len(update_events)} updates, {len(delete_events)} deletions, {len(time_blocks)} time blocks")
        ...
        confirm = input(f"Apply these calendar changes from {calendar_file}? (y/n): ").lower().strip()
        ...
        if delete_events:
            ...
            for event_info in delete_events:
                ...
                if event_id and calendar_client.delete_event(calendar_id, event_id, summary):
                    success_count += 1
        ...
        if update_events:
            ...
            for event_info in update_events:
                event_id = event_info.get('event_id')
                if not event_id:
                    print(f"⚠️ Missing event_id for update: {event_info.get('summary', 'Unknown')}")
                    continue
        ...
        if time_blocks:
            ...
            for block_info in time_blocks:
                ...
                if start_time and calendar_client.create_task_time_block(task_data, start_time, duration, calendar_id):
                    success_count += 1
...
def process_calendar_operations():
    ...
    for file in general_files:
        try:
            with open(file, 'r') as f:
                ...
                if any(keyword in content for keyword in ['calendar', 'event', 'time_block', 'schedule']):
    ...
    if not all_calendar_files:
        ...
        print("Expected files: calendar_*.json, events_*.json, schedule_*.json, timeblock_*.json")
    ...
    print(f"🎉 COMPLETE: Successfully processed {total_success} calendar operations across {len(all_calendar_files)} {files_text}!")
