...
def analyze_by_project(tasks, project_names, section_names):
    ...
    project_analysis = defaultdict(lambda: {
        'total': 0,
...
def identify_actionable_opportunities(categories, project_analysis):
    ...
    for task in categories['no_due_date']:
        ...
        if any(word in content for word in ['call', 'email', 'book', 'schedule', 'check', 'update', 'cancel']):
    ...
    opportunities['context_batching'] = {k: v for k, v in label_groups.items() if len(v) >= 2}
...
def display_comprehensive_analysis(tasks, project_names, section_names):
    ...
    for project, analysis in sorted(project_analysis.items(), key=lambda x: x[1]['total'], reverse=True):
        ...
        if len([p for p in priority_counts.values() if p > 0]) > 1:
            priority_str = ", ".join([f"P{p}:{c}" for p, c in sorted(priority_counts.items(), reverse=True) if c > 0])
...
def save_comprehensive_tasks_json(tasks, project_names, section_names, categories, project_analysis, opportunities):
    ...
    claude_data = {
        ...
        "time_categories": {
            ...
            "due_tomorrow": [simplify_task(task) for task in categories['due_tomorrow']],
            "due_this_week": [simplify_task(task) for task in categories['due_this_week']],
            "due_next_week": [simplify_task(task) for task in categories['due_next_week']],
            "due_this_month": [simplify_task(task) for task in categories['due_this_month']],
        ...
        "opportunities": {
            ...
            "neglected_important": [simplify_task(task) for task in opportunities['neglected_important']],
            "context_batching": {
                label: [simplify_task(task) for task in tasks]
                for label, tasks in opportunities['context_batching'].items()
            },
            "project_momentum": [simplify_task(task) for task in opportunities['project_momentum']]
        }
    }
    ...
    print(f"📁 File: local_data/personal_data/all_tasks_comprehensive.json")
...
def main():
    ...
    try:
        ...
        save_comprehensive_tasks_json(
            tasks, project_names, section_names, 
            categories, project_analysis, opportunities
        )
        
        print(f"\n🎉 Comprehensive analysis complete!")
        print(f"📊 Analyzed {len(tasks)} total tasks across {len(project_analysis)} projects")
        print(f"🎯 Identified {len(opportunities['quick_wins'])} quick wins and {len(opportunities['neglected_important'])} high-priority items")
        ...
        print(f"  • High priority without dates: {len(opportunities['neglected_important'])}")
        
        if categories['overdue']:
            print(f"  • ⚠️ URGENT: {len(categories['overdue'])} overdue tasks need attention!")
